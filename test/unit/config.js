...
describe('config', function () {
    describe('.merge', function () {
        ...
        it('honors false as an explicit blacklisting', function () {
            var merged = config.merge({ plugins: { foo: {} } }, { plugins: { foo: false } }, { plugins: { foo: {}, bar: {} } });
    ...
    describe('.expand', function () {
        describe('deprecated options', function () {
            it('expands local string browsers', function () {
                var context = new context_1.Context({ browsers: ['chrome'] });
                return config.expand(context).then(function () {
            ...
            it('expands sauce string browsers', function () {
                var context = new context_1.Context({ browsers: ['linux/firefox'] });
                return config.expand(context).then(function () {
            ...
            it('expands local object browsers', function () {
                var context = new context_1.Context({ browsers: [{ browserName: 'firefox' }] });
                return config.expand(context).then(function () {
            ...
            it('expands sauce object browsers', function () {
                var context = new context_1.Context({ browsers: [{ browserName: 'safari', platform: 'OS X' }] });
                return config.expand(context).then(function () {
